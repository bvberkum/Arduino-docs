...
#define SHT1X 0     // 1 for SHT11/SHT15, 0 for 1-Wire DS18B20
...
// In that case, the SHT11 or DS18B20 connexts to port 1.
...
//AT: Freshmatic LED is connected with analog pin port4 (normally used for SHT11)
Port fresh_led (4);
...
void blinkLed(byte count, int interval=5)
{
    for (byte i = 0; i < count; ++i) {
    	if (i > 0)
			delay(interval);
		fresh_led.digiWrite2(0);
		delay(interval);
		fresh_led.digiWrite2(1);
	}
}
...
static void newReadings() {
    ...
    if ( payload.moved = motion != prevMotion ) {
      //flash when moved it true
      blinkLed(2,55);
    }
    prevMotion = motion;
...
void setup() {
    ...
    //AT: send startup flash on Freshmatic LED
    fresh_led.mode2(OUTPUT);
    fresh_led.digiWrite2(0);
