...
void ser_configNodeCmd(void) {
  //cmdIo.println("n " + node_id);
  //cmdIo.print('n');
  //cmdIo.print(' ');
  //cmdIo.print(node_id);
  //cmdIo.print('\n');
...
void ser_configVersionCmd(void) {
  //cmdIo.print("v ");
...
void process_wire_cmd(char req, int fill, char *buffer) {
  cmdIo.println(sizeof wirecmd_tab);
//  const WireCommand p;
//  for (p = cmds; ; ++p) {
...
void doReport(void)
{
  ...
  cmdIo.print(node);
  cmdIo.print(" ");
  cmdIo.print(pir.state());
  cmdIo.print(" ");
  cmdIo.print(pir.triggered());
  cmdIo.println("");
#endif
}
