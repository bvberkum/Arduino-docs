...
int __attribute__((noreturn)) main(void)
{
    ...
    if(bootLoaderCondition()){
        ...
        /* turn on green LED to signal boot loader is ready for upload */
        DDRC |= (1 << PC0);
        do{
            usbPoll();
#if TIMEOUT_ENABLED
...
#endif
        }while(bootLoaderCondition() || USING_PUSHBUTTON); /* main event loop */
    }

    /* moved delay from if(requestBootLoaderExit) block. this delay also causes
       the red LED to flash indicating the boot loader timed out. */
    _delay_ms(100);
    leaveBootloader();
}
